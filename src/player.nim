import client
export cE  # Exporting the error checking macro

type
  PlayerError* = object of CatchableError
  MediaInfo* = object
    title*: string
    isIdle*: bool
    volume*: int
...
proc validateVolume(volume: int): int =
  ## Ensures volume stays within valid bounds
...
proc pause*(ctx: ptr Handle, shouldPause: bool) {.raises: [PlayerError].} =
  ## Toggles pause state of the player
  ## 
  ## Args:
  ##   ctx: Player handle
...
proc adjustVolume*(ctx: ptr Handle, increase: bool): int {.raises: [PlayerError].} =
  ## Adjusts volume up or down by VolumeStep
  ## 
  ## Args:
  ##   ctx: Player handle
...
proc observeMediaTitle*(ctx: ptr Handle) {.raises: [PlayerError].} =
  ## Starts observing media title changes
  ## 
  ## Args:
  ##   ctx: Player handle
  try:
...
proc isIdle*(ctx: ptr Handle): bool {.raises: [PlayerError].} =
  ...
  ## 
  ## Returns:
  ##   True if player is idle, False otherwise
  try:
    var idleState: cint
...
proc getCurrentMediaTitle*(ctx: ptr Handle): string {.raises: [PlayerError].} =
  ## Gets the current media title
  ## 
  ## Args:
  ##   ctx: Player handle
  ## 
  ## Returns:
  ##   Current media title or empty string if none
  try:
    var title: cstring
...
proc getMediaInfo*(ctx: ptr Handle): MediaInfo {.raises: [PlayerError].} =
  ## Gets comprehensive media player state
  ## 
  ## Args:
  ##   ctx: Player handle
  ## 
  ## Returns:
  ##   MediaInfo object containing current player state
  try:
    var 
      volume: int
      muteState: cint
